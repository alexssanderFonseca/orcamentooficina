...
Microserviço responsável pela criação, aprovação e gerenciamento de orçamentos para ordens de serviço.

---

## 📋 Descrição

O **Serviço de Gerenciamento de Orçamentos** é um microserviço construído em **Java 21** com **Spring Boot** e **PostgreSQL**. A sua arquitetura é baseada no padrão **Ports and Adapters (Arquitetura Hexagonal)**, que isola a lógica de negócio de detalhes de infraestrutura.

O sistema foi projetado para simular processos típicos de uma oficina, como:

-   Criação e aprovação de orçamentos
-   Gerenciamento de itens de orçamento
-   Comunicação com o microserviço de Ordem de Serviço
...
- [**Collection Insomnia - Orçamento**](docs/collections/orcamento-insomnia-collection.yaml) <!-- Placeholder: Update with actual path if exists -->
...
1.  Navegue até o diretório `orcamento/`.
...
2.  Construa a imagem Docker do microserviço:
    ...
    ```
    (Substitua `alexmarquesfa/orcamentooficina` pelo nome da sua imagem no Docker Hub, se aplicável).
...
Após alguns instantes, todos os recursos (Pods, Services, Deployments, etc.) para o serviço de `orcamento` estarão sendo criados.
...
O projeto utiliza GitHub Actions para automação de integração e deploy contínuo. Os workflows específicos para este microserviço estão definidos em `.github/workflows` dentro do diretório `orcamento/`.

### CI - Integração Contínua (`orcamento/.github/workflows/ci-pipeline.yml`)

Este pipeline é acionado a cada `push` em uma branch com o padrão `feature/*` e quando há alterações no diretório `orcamento/`. Seu objetivo é garantir a qualidade e a integridade do código antes que ele seja mesclado à `main`.
...
### CD - Deploy Contínuo (`orcamento/.github/workflows/cd-pipeline.yml`)

Este pipeline é acionado automaticamente após um `merge` na branch `main` e quando há alterações no diretório `orcamento/`. Seu objetivo é colocar a nova versão da aplicação em produção no ambiente da AWS.
